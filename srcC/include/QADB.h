...
private:
    ...
    bool chargeHLCounted;
    std::vector<std::pair<int,int>> chargeHLCountedFiles;
    
    std::map<int,double> chargeHL;
    std::map<int,double> chargeHLTotal;
...
QADB::QADB(std::string const& cook, int runnumMin_, int runnumMax_, bool verbose_) {
    ...
    chargeHL.insert(std::pair<int,double>(0,-1.0));
    chargeHL.insert(std::pair<int,double>(1,-1.0));
    ...
    if( runnum_ != runnum || filenum_ != filenum) {
        ...
        if(qaTree.HasMember(runnumStr)) {
            ...
            if(runTree.HasMember(filenumStr)) {
                ...
                if(chargeTree[runnumStr][filenumStr].HasMember("fcChargeHelicity")){
                    for(auto it=chargeTree[runnumStr][filenumStr]["fcChargeHelicity"].MemberBegin(); it!=chargeTree[runnumStr][filenumStr]["fcChargeHelicity"].MemberEnd(); ++it) {
                        chargeHL[atoi((it->name).GetString())] = (it->value).GetDouble();
                    }
                    chargeHLCounted = false;
                };
                found = true;
...
//..................................................
// Helicity Latched Faraday Cup charge accumulator
//``````````````````````````````````````````````````
// -- accumulator
// call this method after evaluating QA cuts (or at least after calling query())
...
void QADB::AccumulateChargeHL() {
    if(!chargeHLCounted) {
        if(
           find(
                chargeHLCountedFiles.begin(),
                chargeHLCountedFiles.end(),
                std::pair<int,int>(runnum,filenum)
                ) == chargeHLCountedFiles.end()
           ) {
               for (auto iter = chargeHL.begin(); iter != chargeHL.end(); ++iter){
                   chargeHLTotal[iter->first] += iter->second;
               }
               chargeHLCountedFiles.push_back(std::pair<int,int>(runnum,filenum));
           }
...
double QADB::GetAccumulatedChargeHL(int state) {
    double ret = -1.0;
...
// reset accumulated charge, if you ever need to
void QADB::ResetAccumulatedChargeHL() {
    for (auto iter = chargeHLTotal.begin(); iter != chargeHLTotal.end(); ++iter){
        chargeHLTotal[iter->first] = 0;
    }
};
